 ...
 *
 **********************************************************************************************************************/
public class BHGEObsoleteProductRefProvider extends AbstractPropertyFieldValueProvider implements FieldValueProvider, Serializable
{

	/**
	 *
	 */
	private final static Logger LOG = Logger.getLogger(BHGEObsoleteProductRefProvider.class);
			...
			throws FieldValueProviderException
	{
		...
		if (model instanceof GEEdgeProductModel)
		{
			...
			final Collection<ProductReferenceModel> productReferenceModels = productReferenceService
					.getProductReferencesForSourceProduct(geEdgeProduct, ProductReferenceTypeEnum.OBSOLETE, Boolean.TRUE);
			if (CollectionUtils.isNotEmpty(productReferenceModels))
			{
				fieldValues.addAll(createFieldValues(indexedProperty, true));
			}
			else
			{
				fieldValues.addAll(createFieldValues(indexedProperty, false));
			}
		}
		return fieldValues;
	}
