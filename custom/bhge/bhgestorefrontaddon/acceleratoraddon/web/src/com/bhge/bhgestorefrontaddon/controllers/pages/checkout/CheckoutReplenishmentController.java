 ...
 */
package com.bhge.bhgestorefrontaddon.controllers.pages.checkout;
...
public class CheckoutReplenishmentController extends AbstractCheckoutController
{
			...
			throws CMSItemNotFoundException
	{
		if (StringUtils.isNotEmpty(jobCode) || StringUtils.isNotBlank(jobCode))
		{
			...
			return processOrderCode(StringEscapeUtils.escapeHtml4(jobCode), model);
		}
		return jobCode;
	}
	...
	protected String processOrderCode(final String jobCode, final Model model) throws CMSItemNotFoundException
	{
		if (StringUtils.isNotEmpty(jobCode) || StringUtils.isNotBlank(jobCode))
		{
			final ScheduledCartData scheduledCartData = b2bOrderFacade.getReplenishmentOrderDetailsForCode(
					StringEscapeUtils.escapeHtml4(jobCode), getCustomerFacade().getCurrentCustomer().getUid());
			...
			model.addAttribute(SCHEDULE_URL_KEY, ACCOUNT_REPLENISHMENT_PAGE + "/" + StringEscapeUtils.escapeHtml4(jobCode));
		}
