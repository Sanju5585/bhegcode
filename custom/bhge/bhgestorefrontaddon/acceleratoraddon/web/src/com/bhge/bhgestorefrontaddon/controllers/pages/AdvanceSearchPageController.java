...
import com.bhge.bhgestorefrontaddon.forms.AdvancedSearchForm;
...
public class AdvanceSearchPageController extends AbstractSearchPageController
{
	...
	public String advanceSearchResults(
			...
			@RequestParam(value = "searchResultType", required = false, defaultValue = ADVANCED_SEARCH_RESULT_TYPE_ORDER_FORM) final String searchResult,
			...
			final HttpServletRequest request) throws CMSItemNotFoundException
	{
		...
		if (StringUtils.isNotBlank(keywords) || StringUtils.isEmpty(keywords))
		{
			searchTerm = StringEscapeUtils.escapeHtml4(keywords);
		}
		if (StringUtils.isNotEmpty(searchResult))
		{
			searchResultType = StringEscapeUtils.escapeHtml4(searchResult);
		}
		if (StringUtils.isEmpty(searchQuery) || StringUtils.isNotEmpty(searchQuery))
		{
			if (StringUtils.isNotBlank(StringEscapeUtils.escapeHtml4(searchQuery)))
			{
				searchTerm = StringUtils.split(StringEscapeUtils.escapeHtml4(searchQuery), ":")[0];
			}
		}
			...
			final PageType pageType, final String robotsBehaviour)
	{
		final String metaDescription = MetaSanitizerUtil
				.sanitizeDescription(getMessageSource().getMessage(metaPrefixKey, null, getCurrentLocale()) + " " + searchText + " "
						+ getMessageSource().getMessage(metaPostfixKey, null, getCurrentLocale()) + " " + getSiteName());
	...
	protected String getPaginationUrlFromHttpRequest(final HttpServletRequest request)
	{
		...
		requestParamMap.entrySet().stream().filter(predicate.negate()).forEach(entry -> queryParamBuilder
				.append(((queryParamBuilder.length() > 1) ? "&" : "") + entry.getKey() + "=" + entry.getValue()[0]));
