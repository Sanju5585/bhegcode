 ...
 */
public class BHGEGenerateQuoteEmailAction extends AbstractSimpleDecisionAction {
    ...
    public Transition executeAction(final BusinessProcessModel businessProcessModel) {
        LOG.info(" ############ Inside the BHGEGenerateQuoteEmailAction ###########");
        ...
        if (contentCatalogVersion == null) {
            contentCatalogVersion = catalogVersionService.getCatalogVersion(BhgeCoreConstants.CONTENT_CATALOG,
                    "Online");
        }
        catalogVersionService.setSessionCatalogVersions(Collections.singletonList(contentCatalogVersion));
        ...
        if (currentBaseStore == null) {
            currentBaseStore = baseStoreService.getBaseStoreForUid("bhge");
        }
        final UserModel quoteUserModel = userService.getCurrentUser();
        String quoteArray[] = businessProcessModel.getCode().split("-");
        String quoteCode = quoteArray[1];
        QuoteModel quoteModel = commerceQuoteDao.findUniqueQuoteByCodeAndCustomerAndStore(currentUser, currentBaseStore, quoteCode,
                quoteStateSelectionStrategy.getAllowedStatesForAction(QuoteAction.VIEW, quoteUserModel));
        if (contentCatalogVersion == null) {
        ...
        if (quoteModel.getAttachments() != null && quoteModel.getAttachments().size() > 0) {
            for (MediaModel file : quoteModel.getAttachments()) {
                if (file != null) {
                    try {
                        ...
                        LOG.error("unable to add attachments from quote:" + quoteModel.getCode() + "with error:"
                                + e.getMessage());
                    }
                }
            }
        ...
        try {
            ...
            quoteAttachment = getQuotePDFAttachment(quoteModel);
        } catch (JAXBException | IOException | URISyntaxException | TransformerException e) {
            LOG.error("US564371 : unable to attach quote as PDF:" + quoteModel.getCode() + "with error:"
                    + e.getMessage());
        }
        ...
        final String QuoteProcessCode = businessProcessModel.getCode();
        final BHGEQuoteProcessModel BHGEQuoteProcessModel = (BHGEQuoteProcessModel) getBusinessProcessService().getProcess(
                QuoteProcessCode);
        String countryCode = null;
        if (BHGEQuoteProcessModel.getCountry() != null) {
            countryCode = BHGEQuoteProcessModel.getCountry().getIsocode();
        }

        final Set<String> countryCodeList = new HashSet<String>(Arrays.asList(new String[]{"US", "CA", "BR", "MX", "CO", "AR", "PE", "VE", "CL", "EC", "GT", "HT", "BO", "DO", "HN", "PY", "SV", "NI", "CR", "PR",
            "PA", "UY", "GP", "MQ", "GF", "MF", "BL"}));
        String bcEmailAddress = "";
        String[] emailIds;
        final String delimiter = ",";
        Set<String> superCategoryCodes = new HashSet<String>();

        if (null != quoteModel.getEntries()) {
            AbstractOrderEntryModel quoteEntry = quoteModel.getEntries().get(0);
            if (null != quoteEntry.getProduct()) {
                List<CategoryModel> superCategories = (List<CategoryModel>) quoteEntry.getProduct().getSupercategories();
                ...
                if (superCategoryCodes.contains(Config.getParameter("waygate.category.code"))) {
                    LOG.info("############### Category selected for Quote is " + Config.getParameter("waygate.category.code"));
                    bcEmailAddress = Config.getParameter("quote.waygate.email");
                }
                if (superCategoryCodes.contains(Config.getParameter("nexus.category.code"))) {
                    LOG.info("############### Category selected for Quote is " + Config.getParameter("nexus.category.code"));
                    bcEmailAddress = Config.getParameter("quote.nexus.email");
                }
                if (superCategoryCodes.contains(Config.getParameter("pana.category.code"))) {
                    LOG.info("############### Category selected for Quote is " + Config.getParameter("pana.category.code"));
                    bcEmailAddress = Config.getParameter("quote.pana.email");
                }
            }
        }
        if (StringUtils.isBlank(bcEmailAddress)) {
            if (countryCodeList.contains(countryCode) || countryCode == null) {
                bcEmailAddress = Config.getParameter("quote.bcEmailUS.default");
            } else {
                bcEmailAddress = Config.getParameter("quote.bcEmailSales.default");
            }
        }
        emailIds = bcEmailAddress.split(delimiter);
        ...
        final List<EmailAddressModel> toEmailIds = new ArrayList<EmailAddressModel>();
        EmailAddressModel toAddress = null;
        for (int i = 0; i < emailIds.length; i++) {
            toAddress = emailService.getOrCreateEmailAddressForEmail(emailIds[i], " ");
            toEmailIds.add(toAddress);
        }
        //final EmailAddressModel toAddress = emailService.getOrCreateEmailAddressForEmail(bcEmailAddress, " ");
        final EmailAddressModel ccAddress = emailService.getOrCreateEmailAddressForEmail(BHGEQuoteProcessModel.getEmailAddress(),
                " ");
        if (countryCode == null) {
            toEmails.add(ccAddress);
            ccEmails.addAll(toEmailIds);
        } else {
            toEmails.addAll(toEmailIds);
            ccEmails.add(ccAddress);
        }
        emailMessageModel.setToAddresses(toEmails);
        emailMessageModel.setCcAddresses(ccEmails);
        emailMessageModel.setBccAddresses(toEmails);
    ...
    private EmailAttachmentModel generateQuotePdf(String xml, String xsltFile, QuoteModel quoteModel) throws URISyntaxException, IOException,
        ...
        try {
            ...
            String quoteFileName = QUOTE + "-" + sdf.format(timestamp) + PDF;
