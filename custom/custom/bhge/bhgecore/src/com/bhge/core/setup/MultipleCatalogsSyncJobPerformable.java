...
import com.bhge.core.model.MultipleCatalogsSyncCronJobModel;
...
/**
 * Abstraction of the dependent {@link CatalogVersionSyncCronJobModel} performs
 * in determined order for a given by {@link MultipleCatalogsSyncCronJobModel}
 * attributes :
 * <ul>
 ...
 */
public class MultipleCatalogsSyncJobPerformable extends
		AbstractJobPerformable<MultipleCatalogsSyncCronJobModel> {
	private static final Logger LOG = Logger
			.getLogger(MultipleCatalogsSyncJobPerformable.class.getName());
	...
	public PerformResult perform(final MultipleCatalogsSyncCronJobModel cronJob) {
		Preconditions
				.checkNotNull(cronJob, "Given cron job should not be null");
		Preconditions.checkArgument(
				!CollectionUtils.isEmpty(cronJob.getContentCatalogs()),
				"Given cron job's content catalog should not be empty");
		Preconditions.checkNotNull(cronJob.getProductCatalog(),
				"Given cron job's product catalog should not be null");
		...
		getSetupSyncJobService().assignDependentSyncJobs(
				cronJob.getProductCatalog().getId(), contentIds);
	...
	protected PerformResult callInternal(final List<String> allCatalogs) {
		PerformResult result = new PerformResult(CronJobResult.SUCCESS,
				CronJobStatus.FINISHED);
		try {
			for (final String singleCall : allCatalogs) {
			   if(!singleCall.equalsIgnoreCase(BhgeCoreConstants.CONTENT_CATALOG)){
					final PerformResult singleResult = getSetupSyncJobService()
							.executeCatalogSyncJob(singleCall);
			...
			LOG.error("One of the cronjob computations caused an exception : "
					+ e);
