...
public class SubmitOrderEventListener extends AbstractAcceleratorSiteEventListener<SubmitOrderEvent>
{
	...
	protected void onSiteEvent(final SubmitOrderEvent event)
	{
		...
		} else if (BooleanUtils.isTrue(order.getIsQuote())) {
			LOG.info("US530529: Order is an Quote Converted Order");
			final QuoteConversionEvent quoteConversionEvent = new QuoteConversionEvent(order);
			eventService.publishEvent(quoteConversionEvent);
		} else {
