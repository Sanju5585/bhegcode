...
public class OrderCancelRepairJobTest
{
	...
	public void cronJobTest()
	{
		...
		when(orderExchangeRepair.findProcessesByActionIds(BhgesaporderfulfillmentConstants.ORDER_PROCESS_NAME, actions)).thenReturn(
				bpList);
		repairJob.perform(cronJobModel);
		verify(orderExchangeRepair).findProcessesByActionIds(BhgesaporderfulfillmentConstants.ORDER_PROCESS_NAME, actions);
	}
