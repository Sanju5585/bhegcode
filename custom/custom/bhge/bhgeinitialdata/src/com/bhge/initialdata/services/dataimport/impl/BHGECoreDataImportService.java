...
/**
 * Implementation to handle specific Core Data Import services to bhge.
 */
public class BHGECoreDataImportService extends CoreDataImportService
{

	/**
	 * Imports store data related to bhge. Imports a site-override impex if available.
	 *
	 * @param extensionName
	...
	protected void importStore(final String extensionName, final String storeName, final String productCatalogName)
	{
		...
		if (getConfigurationService().getConfiguration().getBoolean("setup.siteoverride", false))
		{
			getSetupImpexService().importImpexFile(
					String.format("/%s/import/coredata/stores/%s/store-carrier-mapping.impex", extensionName, storeName), false);
			getSetupImpexService().importImpexFile(
					String.format("/%s/import/coredata/stores/%s/store-contactsetting-mapping.impex", extensionName, storeName),
					false);
			getSetupImpexService().importImpexFile(
