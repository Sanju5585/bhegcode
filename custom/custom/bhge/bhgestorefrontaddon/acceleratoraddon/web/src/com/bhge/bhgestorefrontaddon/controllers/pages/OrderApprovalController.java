...
import com.bhge.bhgestorefrontaddon.forms.OrderApprovalDecisionForm;
...
public class OrderApprovalController extends AbstractSearchPageController
{
			...
			throws CMSItemNotFoundException
	{
		try
		{
			B2BOrderApprovalData orderApprovalDetails = null;

			if (StringUtils.isNotEmpty(workflowActionCode) || StringUtils.isNotBlank(workflowActionCode))
			{
				orderApprovalDetails = orderFacade.getOrderApprovalDetailsForCode(StringEscapeUtils.escapeHtml4(workflowActionCode));
			}
			...
			final Model model) throws CMSItemNotFoundException
	{
		try
		{
					...
					&& StringUtils.isEmpty(orderApprovalDecisionForm.getComments()))
			{
				...
				return orderApprovalDetails(StringEscapeUtils.escapeHtml4(orderApprovalDecisionForm.getWorkFlowActionCode()), model);
			}
