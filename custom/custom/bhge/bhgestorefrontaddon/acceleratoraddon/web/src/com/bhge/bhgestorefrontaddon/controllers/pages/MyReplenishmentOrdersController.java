...
import com.bhge.bhgestorefrontaddon.forms.ReorderForm;
...
public class MyReplenishmentOrdersController extends AbstractSearchPageController
{
	...
	{
		if ((StringUtils.isNotEmpty(jobCode) && StringUtils.isNotBlank(jobCode)) && request.getContentLength() != 0)
		{
			...
			breadcrumbs.add(new Breadcrumb("#",
					...
					{ StringEscapeUtils.escapeHtml4(jobCode) }, "Remove Replenishment Schedule {0}",
							getI18nService().getCurrentLocale()),
					null));

			model.addAttribute("breadcrumbs", breadcrumbs);
			model.addAttribute("arguments", String.format("%s", StringEscapeUtils.escapeHtml4(jobCode)));
			model.addAttribute("page", "replenishment");
			model.addAttribute("disableUrl", String.format("%s/my-account/my-replenishment/cancel/%s", StringEscapeUtils.escapeHtml4(request.getContextPath()),
					StringEscapeUtils.escapeHtml4(jobCode)));
			model.addAttribute("cancelUrl", String.format("%s/my-account/my-replenishment/%s", StringEscapeUtils.escapeHtml4(request.getContextPath()),
					StringEscapeUtils.escapeHtml4(jobCode)));
		}
	...
	{
		if (StringUtils.isNotEmpty(jobCode) && StringUtils.isNotBlank(jobCode))
		{
			final ScheduledCartData scheduledCartData = orderFacade.getReplenishmentOrderDetailsForCode(
					StringEscapeUtils.escapeHtml4(jobCode), customerFacade.getCurrentCustomer().getUid());
			model.addAttribute("orderData", scheduledCartData);

			final PageableData pageableData = createPageableData(page, 5, sortCode, showMode);
			final SearchPageData<? extends OrderHistoryData> searchPageData = orderFacade
					.getPagedReplenishmentOrderHistory(StringEscapeUtils.escapeHtml4(jobCode), pageableData);
			populateModel(model, searchPageData, showMode);
			...
			breadcrumbs.add(new Breadcrumb("/my-account/my-replenishment",
					...
					null));
			breadcrumbs.add(
					new Breadcrumb(String.format("/my-account/my-replenishment/%s/", urlEncode(StringEscapeUtils.escapeHtml4(jobCode))),
							getMessageSource().getMessage("text.account.replenishment.replenishmentBreadcrumb", new Object[]
							{ StringEscapeUtils.escapeHtml4(jobCode) }, "Replenishment Schedule {0}",
									getI18nService().getCurrentLocale()),
							null));
			model.addAttribute("breadcrumbs", breadcrumbs);
		}
	...
	public String confirmCancelReplenishmentFromDetailsPage(@PathVariable("jobCode") final String jobCode, final Model model,
			final HttpServletRequest request) throws CMSItemNotFoundException
	{
		if ((StringUtils.isNotBlank(jobCode) && StringUtils.isNotEmpty(jobCode)) && request.getContentLength() != 0)
		{
			final List<Breadcrumb> breadcrumbs = accountBreadcrumbBuilder.getBreadcrumbs(null);
			breadcrumbs.add(new Breadcrumb("/my-account/my-replenishment",
					getMessageSource().getMessage("text.account.manageReplenishment", null, getI18nService().getCurrentLocale()),
					null));
			breadcrumbs.add(new Breadcrumb(String.format("/my-account/my-replenishment/%s/", StringEscapeUtils.escapeHtml4(jobCode)),
					getMessageSource().getMessage("text.account.replenishment.replenishmentBreadcrumb", new Object[]
					{ StringEscapeUtils.escapeHtml4(jobCode) }, "Replenishment Orders {0}", getI18nService().getCurrentLocale()),
					null));
			breadcrumbs.add(new Breadcrumb("#",
					getMessageSource().getMessage("text.account.manageReplenishment.confirm.cancel.breadcrumb", new Object[]
					{ StringEscapeUtils.escapeHtml4(jobCode) }, "Remove Replenishment Schedule {0}",
			...
			model.addAttribute("arguments", String.format("%s", StringEscapeUtils.escapeHtml4(jobCode)));
			model.addAttribute("page", "replenishment");
			model.addAttribute("disableUrl", String.format("%s/my-account/my-replenishment/detail/cancel/%s",
					StringEscapeUtils.escapeHtml4(request.getContextPath()), StringEscapeUtils.escapeHtml4(jobCode)));
			model.addAttribute("cancelUrl", String.format("%s/my-account/my-replenishment/%s",
					StringEscapeUtils.escapeHtml4(request.getContextPath()), StringEscapeUtils.escapeHtml4(jobCode)));
		}
		storeCmsPageInModel(model, getContentPageForLabelOrId(MY_REPLENISHMENT_CANCEL_CONFIRMATION_CMS_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(MY_REPLENISHMENT_CANCEL_CONFIRMATION_CMS_PAGE));
			...
			@PathVariable("orderCode") final String orderCode, final Model model) throws CMSItemNotFoundException
	{
		try
		{
			if (StringUtils.isNotEmpty(jobCode) && StringUtils.isNotEmpty(orderCode))
			{
				model.addAttribute("orderData", orderFacade.getOrderDetailsForCode(StringEscapeUtils.escapeHtml4(orderCode)));
				model.addAttribute("scheduleData", orderFacade.getReplenishmentOrderDetailsForCode(
						StringEscapeUtils.escapeHtml4(jobCode), customerFacade.getCurrentCustomer().getUid()));
				...
				breadcrumbs.add(new Breadcrumb("/my-account/my-replenishment",
						...
						null));
				breadcrumbs
						.add(new Breadcrumb(String.format("/my-account/my-replenishment/%s/", StringEscapeUtils.escapeHtml4(jobCode)),
								getMessageSource().getMessage("text.account.replenishment.replenishmentBreadcrumb", new Object[]
								{ StringEscapeUtils.escapeHtml4(jobCode) }, "Replenishment {0}", getI18nService().getCurrentLocale()),
								null));
				breadcrumbs.add(new Breadcrumb(
						String.format("/my-account/my-replenishment/%s/%s/", StringEscapeUtils.escapeHtml4(jobCode),
								StringEscapeUtils.escapeHtml4(orderCode)),
						getMessageSource().getMessage("text.account.replenishment.replenishmentOrderDetailBreadcrumb", new Object[]
						{ StringEscapeUtils.escapeHtml4(orderCode) }, "Order {0}", getI18nService().getCurrentLocale()), null));
				model.addAttribute("breadcrumbs", breadcrumbs);
			}
		}
	...
	{
		if (StringUtils.isNotEmpty(jobCode) && StringUtils.isNotEmpty(productCode))
		{
			final ScheduledCartData scheduledCartData = orderFacade.getReplenishmentOrderDetailsForCode(
					StringEscapeUtils.escapeHtml4(jobCode), customerFacade.getCurrentCustomer().getUid());

			final Map<String, ReadOnlyOrderGridData> readOnlyMultiDMap = orderGridFormFacade.getReadOnlyOrderGridForProductInOrder(
					StringEscapeUtils.escapeHtml4(productCode), Arrays.asList(ProductOption.BASIC, ProductOption.CATEGORIES),
					scheduledCartData);
			model.addAttribute("readOnlyMultiDMap", readOnlyMultiDMap);
		}
		return BhgestorefrontaddonControllerConstants.Views.Pages.Checkout.ReadOnlyExpandedOrderForm;
	}
}
