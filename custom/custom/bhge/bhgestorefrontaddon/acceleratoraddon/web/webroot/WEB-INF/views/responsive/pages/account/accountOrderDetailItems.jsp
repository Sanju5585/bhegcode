...
<div class="col-md-12 col-lg-12 account-orderdetail account-consignment split-orderID${count.index}" <c:if test="${count.index gt 0}">style="display:none"</c:if>>
    <ycommerce:testId code="orderDetail_itemList_section">
        <c:if test="${not empty orderData.unconsignedEntries}">
            <b2b-order:orderUnconsignedEntries order="${orderData}" count="${count.index}"/>
        </c:if>
