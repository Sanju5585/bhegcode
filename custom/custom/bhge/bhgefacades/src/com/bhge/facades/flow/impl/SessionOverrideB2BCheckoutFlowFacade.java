...
/**
 * Specialised version of the DefaultB2BCheckoutFlowFacade that allows the
 * checkout flow and pci strategy to be overridden in the session. This is
 * primarily used for demonstration purposes and you may not need to use this
 * sub-class in your environment.
 */
public class SessionOverrideB2BCheckoutFlowFacade extends
		DefaultB2BCheckoutFlowFacade {
	private static final Logger LOG = Logger
			.getLogger(SessionOverrideB2BCheckoutFlowFacade.class);
	...
	public B2BCheckoutFlowEnum getCheckoutFlow() {
		final B2BCheckoutFlowEnum sessionOverride = getSessionService()
				.getAttribute(B2B_SESSION_KEY_CHECKOUT_FLOW);
	...
	public B2BCheckoutPciOptionEnum getSubscriptionPciOption() {
		final B2BCheckoutPciOptionEnum sessionOverride = getSessionService()
				.getAttribute(B2B_SESSION_KEY_SUBSCRIPTION_PCI_OPTION);
		if (sessionOverride != null) {
			LOG.info("B2B Session Override SubscriptionPciOption ["
					+ sessionOverride + "]");
			return sessionOverride;
		}
	...
	public static void setSessionOverrideCheckoutFlow(
			final B2BCheckoutFlowEnum checkoutFlow) {
		getStaticSessionService().setAttribute(B2B_SESSION_KEY_CHECKOUT_FLOW,
				checkoutFlow);
	}

	public static void setSessionOverrideSubscriptionPciOption(
			final B2BCheckoutPciOptionEnum checkoutPciOption) {
		getStaticSessionService().setAttribute(
				B2B_SESSION_KEY_SUBSCRIPTION_PCI_OPTION, checkoutPciOption);
	}
