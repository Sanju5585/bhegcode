 ...
 */
package de.hybris.platform.bhgeticketingaddon.controllers.pages;
...
import de.hybris.platform.bhgeticketingaddon.forms.SupportTicketForm;
...
public class RegisterSupportTicketsPageController extends AbstractSearchPageController
{
	private static final Logger LOG = Logger.getLogger(RegisterSupportTicketsPageController.class);

	// CMS Pages
	private static final String SUPPORT_TICKET_CODE_PATH_VARIABLE_PATTERN = "{ticketId:.*}";
	private static final String REDIRECT_TO_SUPPORT_TICKETS_PAGE = REDIRECT_PREFIX + "/register/support-tickets";
	...
	{
		storeCmsPageInModel(model, getContentPageForLabelOrId(BhgeticketingaddonConstants.SUPPORT_TICKETS_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(BhgeticketingaddonConstants.SUPPORT_TICKETS_PAGE));
		model.addAttribute(WebConstants.BREADCRUMBS_KEY,
				accountBreadcrumbBuilder.getBreadcrumbs(BhgeticketingaddonConstants.TEXT_SUPPORT_TICKETING_HISTORY));
		...
		if (ticketAdded)
		{
			GlobalMessages.addConfMessage(model, BhgeticketingaddonConstants.TEXT_SUPPORT_TICKETING_ADDED);
		}
	...
	{
		storeCmsPageInModel(model, getContentPageForLabelOrId(BhgeticketingaddonConstants.ADD_SUPPORT_TICKET_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(BhgeticketingaddonConstants.ADD_SUPPORT_TICKET_PAGE));

		//	model.addAttribute(WebConstants.BREADCRUMBS_KEY, getBreadcrumbs(BhgeticketingaddonConstants.TEXT_SUPPORT_TICKETING_ADD));

		final CustomerData customerData = customerFacade.getCurrentCustomer();
		SupportTicketForm supportTicketForm = new SupportTicketForm();
		if(customerData != null && customerData.getUid() != null && !StringUtils.equalsIgnoreCase(customerData.getUid(), "anonymous")) {
			BHGERegieterCustomerModel customerModel =  bhgeUserProfileDao.getRegisterCustomerModelFromUid(customerData.getUid());
			String contactName = customerModel != null && customerModel.getGivenName()!= null ? customerModel.getGivenName() : "";
		...
		model.addAttribute(ThirdPartyConstants.SeoRobots.META_ROBOTS, ThirdPartyConstants.SeoRobots.NOINDEX_NOFOLLOW);
		model.addAttribute(BhgeticketingaddonConstants.SUPPORT_TICKET_FORM, supportTicketForm);
		model.addAttribute(BhgeticketingaddonConstants.MAX_UPLOAD_SIZE, Long.valueOf(maxUploadSizeValue));
		model.addAttribute(BhgeticketingaddonConstants.MAX_UPLOAD_SIZE_MB, FileUtils.byteCountToDisplaySize(maxUploadSizeValue));
		try
		{
			model.addAttribute(BhgeticketingaddonConstants.SUPPORT_TICKET_ASSOCIATED_OBJECTS, ticketFacade.getAssociatedToObjects());
			model.addAttribute(BhgeticketingaddonConstants.SUPPORT_TICKET_CATEGORIES, ticketFacade.getTicketCategories());
		}
	...
	{
		LOG.info("Start addSupportTicket() method");
		...
		if (bindingResult.hasErrors())
		{
			...
			list.add(
					buildMessageMap(BhgeticketingaddonConstants.FORM_GLOBAL_ERROR_KEY, BhgeticketingaddonConstants.FORM_GLOBAL_ERROR));
		...
		try
		{
			bhgeTicketFacade.bhgeCreateTicket(this.populateTicketData(supportTicketForm));
		...
		catch (final UnsupportedAttachmentException usAttEx)
		{
			...
			map.put(BhgeticketingaddonConstants.SUPPORT_TICKET_TRY_LATER, getMessageSource()
					.getMessage(BhgeticketingaddonConstants.TEXT_SUPPORT_TICKETING_ATTACHMENT_BLOCK_LISTED, new Object[]
		...
		catch (final RuntimeException rEX)
		{
			...
			map.put(BhgeticketingaddonConstants.SUPPORT_TICKET_TRY_LATER, getMessageSource().getMessage(
					BhgeticketingaddonConstants.TEXT_SUPPORT_TICKETING_TRY_LATER, null, getI18nService().getCurrentLocale()));
			return new ResponseEntity<>(Lists.newArrayList(map), HttpStatus.BAD_REQUEST);
		}

		LOG.info("End addSupportTicket() method");
	...
	{
		storeCmsPageInModel(model, getContentPageForLabelOrId(BhgeticketingaddonConstants.UPDATE_SUPPORT_TICKET_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(BhgeticketingaddonConstants.UPDATE_SUPPORT_TICKET_PAGE));

		model.addAttribute(WebConstants.BREADCRUMBS_KEY, getBreadcrumbs(BhgeticketingaddonConstants.TEXT_SUPPORT_TICKETING_UPDATE));
		model.addAttribute(ThirdPartyConstants.SeoRobots.META_ROBOTS, ThirdPartyConstants.SeoRobots.NOINDEX_NOFOLLOW);
		model.addAttribute(BhgeticketingaddonConstants.SUPPORT_TICKET_FORM, new SupportTicketForm());
		model.addAttribute(BhgeticketingaddonConstants.MAX_UPLOAD_SIZE, Long.valueOf(maxUploadSizeValue));
		model.addAttribute(BhgeticketingaddonConstants.MAX_UPLOAD_SIZE_MB, FileUtils.byteCountToDisplaySize(maxUploadSizeValue));
		try
		{
			if(StringUtils.isNotBlank(ticketId) || StringUtils.isNotEmpty(ticketId))
			{
				final TicketData ticketData = ticketFacade.getTicket(XSSEncoder.encodeHTML(StringEscapeUtils.escapeHtml4(ticketId)));
				model.addAttribute(BhgeticketingaddonConstants.SUPPORT_TICKET_DATA, ticketData);
			}
		}
		catch (final Exception e)
		{
			...
			GlobalMessages.addFlashMessage(redirectModel, GlobalMessages.ERROR_MESSAGES_HOLDER,
					BhgeticketingaddonConstants.TEXT_SUPPORT_TICKETING_TRY_LATER, null);
	...
	protected TicketData populateTicketData(final SupportTicketForm supportTicketForm) // TODO: should be moved to populator class
	{
		...
		ticketData.setName(supportTicketForm.getName());
		ticketData.setEmailId(supportTicketForm.getEmailId());
		ticketData.setPhoneNo(supportTicketForm.getPhoneNo());
		return ticketData;
	}
	...
	protected List<Breadcrumb> getBreadcrumbs(final String breadcrumbCode)
	{
		...
		breadcrumbs.add(new Breadcrumb("/register/support-tickets", getMessageSource().getMessage(
				BhgeticketingaddonConstants.TEXT_SUPPORT_TICKETING_HISTORY, null, getI18nService().getCurrentLocale()), null));
		breadcrumbs.add(
