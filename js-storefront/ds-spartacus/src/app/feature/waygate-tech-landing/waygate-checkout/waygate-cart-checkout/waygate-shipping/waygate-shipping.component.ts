...
@Component({
  standalone: false,
  selector: 'app-waygate-shipping',
  templateUrl: './waygate-shipping.component.html',
  styleUrls: ['./waygate-shipping.component.scss'],
})
export class WaygateShippingComponent {
  ...
  ngOnInit(): void {
    ...
    if (this.profileType === CustomerType.Type1) {
      this.disableCollect = true;
      this.deliveryOption = 'Prepay & Add';
    }
    this.onChange();
    if (this.deliveryOption === 'collect') {
      this.showCarrierText = true;
    }
  
    this.sharedCartService.setShippingContactDetails({
      ShipName: this.ShipName,
      shipContact: this.shipContact,
      shipNotimail: this.shipNotimail
    });
  ...
  ngOnChanges() {
    ...
    if (this.checkoutData?.cartData) {
      ...
      if (this.checkoutData?.cartData?.deliveryOptions === 'Prepay & Add') {
      ...
      this.shipContact = this.sharedCartService.formatPhoneNumberTo16Char(
        currentUrl,
        this.checkoutData?.cartData?.shipToContactPhone
      );;
  ...
  onChange(e?, field?) {
    if (field === 'ShipName') {
      ...
      const remarkPattern = /^[\u4e00-\u9fa5\p{L}\p{N}wÁ-ÿa-zA-Z0-9,。+-:/#*&!_.%;・()@ ]+$/u;
      if (this.ShipName && !this.ShipName.match(remarkPattern)) {
        this.error.ShipName = this.getTranslatedText('errors.shipContactName');
      } else {
        this.error.ShipName = '';
      }
    ...
    if (field === 'deliveryPoint') {
      ...
      const deliveryPattern = /^[\u4e00-\u9fa5\p{L}\p{N}wÁ-ÿa-zA-Z0-9,。+-:/#*&!_.%;・()@ ]+$/u;
    ...
    if (field === 'collectText') {
      ...
      const pattern = /^([\u4e00-\u9fa5\p{L}\p{N}a-zA-Z0-9・。]+)$/u;
      if (e.target.value && !e.target.value.match(pattern)) {
  ...
  validateInput(shippingRemark: string) {
    const reasonPattern = /^[\u4e00-\u9fa5\p{L}\p{N}\w\sÁ-ÿ,。+-:/#*&!_.%;・()@ ]+$/mu;
